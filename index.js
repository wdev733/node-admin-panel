...
app.use(function (req, res, next) {
	if(req.signedCookies.auth){
		jwt.verify(req.signedCookies.auth, 'secret', { subject:'admin',issuer:'pihole',audience:'piholeuser' }, function(err, decoded) {
			if(decoded){
		...
		if(req.query.list=="white"||req.query.list=="black"){
			...
			fs.access(filepath, fs.constants.F_OK | fs.constants.R_OK, (err) => {
				if(err){
					...
					lines=[];
					var lineReader = require('readline').createInterface({input: require('fs').createReadStream('/etc/pihole/'+req.query.list+'list.txt')});
					lineReader.on('line', function (line) {
...
app.get('/login', function (req, res) {
	res.render('login_layout.pug', { PCONFIG:{boxedLayout:false,wrongPassword:false,authenticated :req.user.authenticated,activePage:"login"} })
});
app.get('/list', function (req, res) {
	...
	if(req.query.l=="white"){
		res.render('list_layout.pug', { PCONFIG:{boxedLayout:false,wrongPassword:false,authenticated :req.user.authenticated,activePage:"login",listType:'white'} });
	}else if(req.query.l=="black"){
		res.render('list_layout.pug', { PCONFIG:{boxedLayout:false,wrongPassword:false,authenticated :req.user.authenticated,activePage:"login",listType:'black'} });
	}else{
		res.render('list_layout.pug', { PCONFIG:{boxedLayout:false,wrongPassword:false,authenticated :req.user.authenticated,activePage:"login",listType:'unknown'} });
	}
});
...
app.get('/', function (req, res) {
	res.render('main_layout.pug', { PCONFIG:{boxedLayout:false,wrongPassword:false,authenticated :req.user.authenticated,activePage:"home"} })
});
...
app.post('/login', function (req, res) {
	...
	if(token){
		...
		if(tokenHash==CONFIG.WEBPASSWORD){
			...
			jwt.sign({ foo: 'bar' },'secret', { expiresIn:'1h',subject:'admin',issuer:'pihole',audience:'piholeuser' },
				function(err,token) {
					if(token){
						res.cookie('auth', token, { expires: new Date(Date.now() + 60*60*1000), httpOnly: true ,signed:true});
						res.redirect("/home");
					}else{
						console.log("error occured");
						res.render('login_layout.pug', {PCONFIG:{boxedLayout:false,wrongPassword:false,authenticated :false,activePage:"login"} })
					}
				});
			return;
		}
	}
	res.render('login_layout.pug', {PCONFIG:{boxedLayout:false,wrongPassword:true,authenticated :req.user.authenticated,activePage:"login"} })
});
